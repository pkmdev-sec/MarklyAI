...
final class SidebarPositionSelector: NSControl {
    ...
    private func setupView() {
        ...
        leftIcon.contentTintColor = NSColor.labelColor.withAlphaComponent(ThemeConstants.Opacity.high)

        leftLabel.translatesAutoresizingMaskIntoConstraints = false
        leftLabel.font = NSFont.systemFont(ofSize: 13, weight: .medium)
        leftLabel.textColor = NSColor.labelColor.withAlphaComponent(ThemeConstants.Opacity.high)
        ...
        rightIcon.contentTintColor = NSColor.labelColor.withAlphaComponent(ThemeConstants.Opacity.high)

        rightLabel.translatesAutoresizingMaskIntoConstraints = false
        rightLabel.font = NSFont.systemFont(ofSize: 13, weight: .medium)
        rightLabel.textColor = NSColor.labelColor.withAlphaComponent(ThemeConstants.Opacity.high)
    ...
    private func updateAppearance(animated: Bool) {
        let updateButton: (NSView, NSImageView, NSTextField, String) -> Void = { button, icon, label, value in
            ...
            if isSelected {
                ...
                backgroundColor = NSColor.labelColor.withAlphaComponent(ThemeConstants.Opacity.extraSubtle)
            } else if isHovered {
                backgroundColor = NSColor.labelColor.withAlphaComponent(ThemeConstants.Opacity.minimal)
            } else {
                backgroundColor = NSColor.labelColor.withAlphaComponent(ThemeConstants.Opacity.subtle)
            }

            // Text and icon color
            let foregroundColor: NSColor
            if isSelected {
                // In dark mode, labelColor is white, so we need the inverse
                // Use systemBackground which is dark in dark mode
                foregroundColor = NSColor.controlBackgroundColor
            } else {
                foregroundColor = NSColor.labelColor.withAlphaComponent(self.isEnabled ? ThemeConstants.Opacity.high : ThemeConstants.Opacity.low)
            }
