...
enum BrowserManager {
    ...
    static func isRunning(bundleId: String) -> Bool {
        ...
        return NSWorkspace.shared.runningApplications.contains { $0.bundleIdentifier?.lowercased() == target }
    }
    ...
    // Cache for browser detection to avoid repeated filesystem lookups
    nonisolated(unsafe) private static var browserCache: [String: Bool] = [:]

    /// Check if a bundle ID belongs to a browser application
    static func isBrowser(bundleId: String) -> Bool {
        let key = bundleId.lowercased()
        if let cached = browserCache[key] { return cached }

        guard let probeURL = URL(string: "http://example.com") else { return false }
        let browserURLs = NSWorkspace.shared.urlsForApplications(toOpen: probeURL)
        let result = browserURLs.contains { url in
            Bundle(url: url)?.bundleIdentifier?.lowercased() == key
        }
