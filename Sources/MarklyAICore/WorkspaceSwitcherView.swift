...
private final class SettingsButton: BaseControl {
    ...
    private func updateAppearance() {
        ...
        if isSelected {
            // Settings selected: subtle dark highlight (15% white on dark background)
...
private final class WorkspaceButton: BaseControl {
    ...
    private let colorId: WorkspaceColorId
    ...
    init(workspaceId: UUID, name: String, colorId: WorkspaceColorId, style: WorkspaceSwitcherView.Style) {
        ...
        setAccessibilityRole(.button)
        setAccessibilityLabel("Switch to workspace: \(name)")
    ...
    private func updateAppearance() {
        layer?.cornerRadius = style.buttonCornerRadius
        let workspaceColor = colorId.color

        if isSelected {
            // Selected: workspace colored border + subtle fill
            layer?.backgroundColor = workspaceColor.withAlphaComponent(0.08).cgColor
            layer?.borderWidth = 1.5
            layer?.borderColor = workspaceColor.cgColor
            titleLabel.textColor = style.selectedTextColor
        } else if isHovered {
            // Hovered: subtle workspace colored border
            layer?.backgroundColor = NSColor.clear.cgColor
            layer?.borderWidth = 1
            layer?.borderColor = workspaceColor.withAlphaComponent(0.5).cgColor
