...
final class SettingsContentViewController: NSViewController {
    ...
    // Color constants
    private let sectionHeaderColor = NSColor.secondaryLabelColor
    private let regularTextColor = NSColor.labelColor
    ...
    private let checkDeadLinksButton = SettingsButton(title: "Check for Dead Links")
    ...
    // Claude AI section
    private let aiToggle = CustomToggle(title: "AI Auto-Organization")
    private let apiKeyField = NSSecureTextField()
    private let apiKeyLabel = NSTextField(labelWithString: "Claude API Key")
    private let testConnectionButton = SettingsButton(title: "Test Connection")
    private let connectionStatusLabel = NSTextField(labelWithString: "")
    private let privacyLabel = NSTextField(labelWithString: "Only URL and page title are sent to Anthropic's API")
    ...
    private func setupUI() {
        ...
        browserPopupContainer.layer?.backgroundColor = NSColor.labelColor.withAlphaComponent(0.08).cgColor
        ...
        if #available(macOS 14.0, *) {
            browserPopup.contentTintColor = NSColor.labelColor.withAlphaComponent(0.80)
        }
        ...
        checkDeadLinksButton.target = self
        checkDeadLinksButton.action = #selector(checkDeadLinks)
        checkDeadLinksButton.translatesAutoresizingMaskIntoConstraints = false
        ...
        // Claude AI Section
        let aiHeader = createSectionHeader("Claude AI")

        // Toggle
        aiToggle.target = self
        aiToggle.action = #selector(aiToggleChanged)
        aiToggle.isOn = UserDefaults.standard.bool(forKey: "aiOrganizationEnabled")
        aiToggle.translatesAutoresizingMaskIntoConstraints = false

        // API key label
        apiKeyLabel.translatesAutoresizingMaskIntoConstraints = false
        apiKeyLabel.font = NSFont.systemFont(ofSize: 12, weight: .regular)
        apiKeyLabel.textColor = NSColor.secondaryLabelColor
        ...
        apiKeyField.backgroundColor = NSColor.labelColor.withAlphaComponent(0.08)
        ...
        testConnectionButton.action = #selector(testAIConnection)
        ...
        connectionStatusLabel.textColor = NSColor.secondaryLabelColor
        ...
        NSLayoutConstraint.activate([
            ...
            // Check Dead Links Button (below Chrome import button)
            checkDeadLinksButton.leadingAnchor.constraint(equalTo: chromeImportButton.leadingAnchor),
            checkDeadLinksButton.topAnchor.constraint(equalTo: chromeImportButton.bottomAnchor, constant: itemSpacing),
            checkDeadLinksButton.trailingAnchor.constraint(equalTo: contentView.trailingAnchor, constant: -horizontalPadding),
            checkDeadLinksButton.heightAnchor.constraint(equalToConstant: 36),

            // Chrome Help Container (centered below Check Dead Links button)
            chromeHelpContainer.centerXAnchor.constraint(equalTo: contentView.centerXAnchor),
            chromeHelpContainer.topAnchor.constraint(equalTo: checkDeadLinksButton.bottomAnchor, constant: controlLabelSpacing),
            ...
            // Claude AI Header
            aiHeader.leadingAnchor.constraint(equalTo: contentView.leadingAnchor, constant: horizontalPadding),
            aiHeader.topAnchor.constraint(equalTo: separator5.bottomAnchor, constant: sectionSpacing),

            // AI Toggle
            aiToggle.leadingAnchor.constraint(equalTo: aiHeader.leadingAnchor),
            aiToggle.topAnchor.constraint(equalTo: aiHeader.bottomAnchor, constant: sectionHeaderSpacing),
            aiToggle.trailingAnchor.constraint(equalTo: contentView.trailingAnchor, constant: -horizontalPadding),
            aiToggle.heightAnchor.constraint(equalToConstant: 28),

            // API Key Label
            apiKeyLabel.leadingAnchor.constraint(equalTo: aiToggle.leadingAnchor),
            apiKeyLabel.topAnchor.constraint(equalTo: aiToggle.bottomAnchor, constant: itemSpacing),

            // API Key Field
            apiKeyField.leadingAnchor.constraint(equalTo: apiKeyLabel.leadingAnchor),
            apiKeyField.topAnchor.constraint(equalTo: apiKeyLabel.bottomAnchor, constant: controlLabelSpacing),
            apiKeyField.trailingAnchor.constraint(equalTo: contentView.trailingAnchor, constant: -horizontalPadding),
            apiKeyField.heightAnchor.constraint(equalToConstant: 36),

            // Test Connection Button
            testConnectionButton.leadingAnchor.constraint(equalTo: apiKeyField.leadingAnchor),
            testConnectionButton.topAnchor.constraint(equalTo: apiKeyField.bottomAnchor, constant: itemSpacing),
            testConnectionButton.trailingAnchor.constraint(equalTo: contentView.trailingAnchor, constant: -horizontalPadding),
            testConnectionButton.heightAnchor.constraint(equalToConstant: 36),

            // Connection Status Label
            connectionStatusLabel.leadingAnchor.constraint(equalTo: testConnectionButton.leadingAnchor),
            connectionStatusLabel.topAnchor.constraint(equalTo: testConnectionButton.bottomAnchor, constant: controlLabelSpacing),

            // Privacy Label
            privacyLabel.leadingAnchor.constraint(equalTo: connectionStatusLabel.leadingAnchor),
            privacyLabel.topAnchor.constraint(equalTo: connectionStatusLabel.bottomAnchor, constant: controlLabelSpacing),
            privacyLabel.trailingAnchor.constraint(equalTo: contentView.trailingAnchor, constant: -horizontalPadding),

            // Separator 6
            separator6.leadingAnchor.constraint(equalTo: contentView.leadingAnchor, constant: horizontalPadding),
            separator6.trailingAnchor.constraint(equalTo: contentView.trailingAnchor, constant: -horizontalPadding),
            separator6.topAnchor.constraint(equalTo: privacyLabel.bottomAnchor, constant: sectionSpacing),
            ...
            versionHeader.topAnchor.constraint(equalTo: separator6.bottomAnchor, constant: sectionSpacing),
    ...
    @objc private func attachSidebarChanged() {
        ...
        if enabled && !WindowAttachmentService.shared.checkAccessibilityPermissions() {
            ...
            alert.informativeText = "MarklyAI needs Accessibility permissions to attach to windows. Please grant access in System Settings."
    ...
    @objc private func openAccessibilitySettings() {
        ...
        alert.informativeText = "Please grant MarklyAI access in System Settings > Privacy & Security > Accessibility, then return to this window."
    ...
    @objc private func checkDeadLinks() {
        guard let appModel = appModel else { return }

        // Guard against concurrent operations
        if checkDeadLinksButton.getIsLoading() {
            return
        }

        checkDeadLinksButton.setLoading(true)
        Task { @MainActor [weak self] in
            guard let self else { return }
            let deadLinks = await LinkHealthService.shared.checkAllLinks(workspaces: appModel.workspaces)
            self.checkDeadLinksButton.setLoading(false)

            if deadLinks.isEmpty {
                let alert = NSAlert()
                alert.messageText = "All Links Healthy"
                alert.informativeText = "No dead links found."
                alert.runModal()
            } else {
                let alert = NSAlert()
                alert.messageText = "Found \(deadLinks.count) Dead Links"
                alert.informativeText = "These links returned 404 or couldn't be reached."
                alert.addButton(withTitle: "Delete Dead Links")
                alert.addButton(withTitle: "Cancel")
                if alert.runModal() == .alertFirstButtonReturn {
                    for id in deadLinks {
                        appModel.deleteNode(id: id)
                    }
                }
            }
        }
    ...
    @objc private func openChromeExportInstructions() {
        if let url = URL(string: "https://pkmdev-sec.github.io/MarklyAI/import-from-chrome") {
    ...
    @objc private func aiToggleChanged() {
        UserDefaults.standard.set(aiToggle.isOn, forKey: "aiOrganizationEnabled")
        updateAIControlStates()
    }

@objc private func testAIConnection() {
        // Save API key if field has content
        ...
        connectionStatusLabel.textColor = NSColor.secondaryLabelColor
        testConnectionButton.setLoading(true)

        Task {
            let result = await AIOrganizationService.shared.testConnection()
            testConnectionButton.setLoading(false)
            ...
            case .failure(let error):
                connectionStatusLabel.stringValue = "Failed: \(error.description.prefix(80))"
                connectionStatusLabel.textColor = NSColor.systemRed
                print("[Settings] Connection test failed: \(error)")
            }
        }
    }

    private func updateAIControlStates() {
        let enabled = aiToggle.isOn
        apiKeyField.isEnabled = enabled
        testConnectionButton.isEnabled = enabled
        apiKeyLabel.alphaValue = enabled ? 1.0 : 0.5
    }
...
private final class SettingsButton: NSButton {
    ...
    private struct Style {
        // Enabled state
        static let baseBackgroundColor = NSColor.labelColor.withAlphaComponent(0.08)
        static let hoverBackgroundColor = NSColor.labelColor.withAlphaComponent(0.15)
        static let textColor = NSColor.labelColor

        // Disabled state
        static let disabledBackgroundColor = NSColor.separatorColor
        static let disabledTextColor = NSColor.tertiaryLabelColor
