...
class BaseControl: NSControl {
    ...
    /// Allows the control to accept first responder status for keyboard navigation.
    override var acceptsFirstResponder: Bool { true }

    /// Configures the focus ring to render as an exterior outline around the control.
    override var focusRingType: NSFocusRingType {
        get { .exterior }
        set { super.focusRingType = newValue }
    }

    /// Draws the focus ring mask as a rounded rectangle matching the control's shape.
    override func drawFocusRingMask() {
        let path = NSBezierPath(roundedRect: bounds, xRadius: ThemeConstants.CornerRadius.small, yRadius: ThemeConstants.CornerRadius.small)
        path.fill()
    }

    /// Returns the bounds for the focus ring mask.
